...
#define GATTS_TABLE_TAG "GATTS_BLE"

#define PROFILE_NUM                 1
#define PROFILE_APP_IDX             0
#define ESP_APP_ID                  0x55
#define SAMPLE_DEVICE_NAME          "ESP32_BLE_ZERO"
...
static uint8_t raw_adv_data[] = {
        ...
        0x03, 0x03, 0xFF, 0x00,
        /* device name */
        0x0f, 0x09, 'E', 'S', 'P', '3', '2','_', 'B', 'L','E','_','Z', 'E','R','O'
};
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GAP_BLE_PHY_UPDATE_COMPLETE_EVT:
            ESP_LOGI(GATTS_TABLE_TAG, "update phy params status = %d, tx_phy = %d, rx_phy = %d",
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            heart_rate_profile_tab[PROFILE_APP_IDX].gatts_if = gatts_if;
            ESP_LOGE(GATTS_TABLE_TAG, "----------- gatts_if %d", gatts_if);
        } else {
